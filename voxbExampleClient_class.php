...
class voxbExampleClient extends webServiceClientUtils {
	...
	public function createMyDataRequest($userId, $rating, $tags, $objectContributors, $objectIdentifierValue, $objectIdentifierType, $objectMaterialType, $objectPublicationYear, $objectTitle, $reviewTitle, $reviewData) {
		...
		$this->change_tag_value($req_obj, "reviewTitle", $reviewTitle);
		$this->change_tag_value($req_obj, "reviewData", $reviewData);
	...
	function extractData($obj, &$data, $key=0) {
	///	print_r($obj);
   foreach ($obj as $k=>$v) {
			if (is_string($k)) {
    ...
    echo "<tr><td>objectIdentifierValue:</td><td><INPUT TYPE='TEXT' NAME='objectIdentifierValue'><td></tr>\n";
   	...
   	echo "<tr><td>objectMaterialType:</td><td><SELECT NAME='objectMaterialType'>\n";
    ...
    echo "<tr><td>reviewTitle:</td><td><INPUT TYPE='TEXT' NAME='reviewTitle'><td></tr>\n";
